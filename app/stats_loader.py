...
class StatsLoader:
    ...
    @staticmethod
    def get_stats_for_display_export(region_code='national', year: int = 2025):
        """
        Get formatted export statistics for frontend display.
        ...
        Returns:
            dict: Formatted export stats with display-friendly values, or None if not available
        """
        stats = StatsCalculator.load_stats(year=year, region_code=region_code, export=True)
        if not stats or stats.get('tariff_type') != 'export':
            return None
        ...
        formatted = {
            ...
            'coverage': stats.get('coverage'),
            'days_covered': stats.get('days_covered', stats.get('days_processed')),
            'best_block_price': stats.get('best_block', {}).get('avg_price_p_per_kwh', 0),
            'worst_block_price': stats.get('worst_block', {}).get('avg_price_p_per_kwh', 0),
            'daily_average_price': stats.get('daily_average', {}).get('avg_price_p_per_kwh', 0),
            'earnings_vs_daily_avg': stats.get('earnings_vs_daily_average', {}).get('annual_earning_gbp', 0),
            'earnings_vs_daily_avg_pct': stats.get('earnings_vs_daily_average', {}).get('earnings_percentage', 0),
            'negative_pricing': {
                'total_slots': stats.get('negative_pricing', {}).get('total_negative_slots', 0),
                'total_hours': stats.get('negative_pricing', {}).get('total_negative_hours', 0),
                'avg_price_p_per_kwh': stats.get('negative_pricing', {}).get('avg_negative_price_p_per_kwh', 0),
